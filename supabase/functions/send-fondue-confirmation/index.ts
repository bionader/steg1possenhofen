...
function buildConfirmationHtml(name: string, anmeldungId: string, manageToken: string, dateFormatted: string): string {
  return `
...
serve(async (req) => {
  ...
  const anmeldRes = await pg(`fondue_anmeldungen?termin_id=eq.${terminId}&status=eq.vorgemerkt&select=id,anmeldung_id,manage_token,customer_name,customer_email`);
    ...
    try {
      const mailRes = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: buildConfirmationHtml(a.customer_name, a.anmeldung_id, a.manage_token, dateFormatted),
        }),
      });
      if (mailRes.ok) {
        await bumpQuota(2);
